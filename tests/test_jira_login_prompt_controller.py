...
from mcp_server.infrastructure.controllers.prompts.jira_login_controller import JiraLoginPromptController
...
class TestJiraLoginPromptRegistration:
    def test_registers_jira_login_prompt(self, prompt_server):
...
class TestJiraLoginPrompt:
    ...
    @pytest.mark.anyio
    async def test_includes_fallback_flow(self):
        controller = JiraLoginPromptController.__new__(JiraLoginPromptController)
