...
ERROR_MESSAGES: dict[type[CollaborationToolException], str] = {
    UserTokensNotFoundException: "User not authenticated with Jira. Please complete the OAuth flow first.",
...
def tool_error_handler(fn: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            return {"error": True, "error_type": type(exc).__name__, "message": user_message, "detail": str(exc)}
        except Exception as exc:
            logger.exception("Unexpected error in tool '%s'", fn.__name__)
            return {"error": True, "error_type": "InternalError", "message": "An unexpected error occurred. The server is still running.", "detail": str(exc)}
